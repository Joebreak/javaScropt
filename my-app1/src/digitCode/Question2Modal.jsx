...
export default function Question2Modal({ isOpen, onClose, onSubmit, gameData }) {
  ...
  const compareDigits = (digit1, digit2, answerData) => {
    ...
    if (value1 > value2) {
      return `${digit1} > ${digit2} (${value1} > ${value2})`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requestData = {
        room: gameData.room,
        questionType: 2, // 問題2：相鄰數字的大小關係
        digit1: selectedPairData.digit1,
        digit2: selectedPairData.digit2,
        round: gameData.currentRound,
        answer: comparisonResult // 如果有答案數據，直接傳送結果
      };

      // 調用API記錄結果
      const response = await fetch('/api/digitcode/question', {
        method: 'POST',
        headers: {
